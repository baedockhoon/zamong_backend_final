				...
				<c:choose>
                  	<c:when test="${empty list }">		                        
                    	...
                    	<c:otherwise>               	
                          ...
                          <td>${item.mv_no}</td>					                             
                          <td>${item.at_no}</td>
                          <td align="left">${item.mv_title}</td>
                          <td>${item.mv_link}</td>
                          <td>${item.mv_content}</td>
                          <td>${item.mv_hitcount}</td>
                          <td>${item.mv_postdate}</td>
                          <td><a href='<c:url value="/ZamongTV/zamongView.do?no=${item.mv_no}"/>'>수정</a> | <a href="#">삭제</a></td>
                       	</tr>
                      	</c:forEach>
                     	</c:otherwise>                           
                 </c:choose>
