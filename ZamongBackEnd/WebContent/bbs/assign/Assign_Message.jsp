...
<c:choose>
	<c:when test="${WHERE eq 'INS' }">
		...
		<c:set var="SUC_URL" value="/bbs/assign/Assigntest.jsp"/>
	</c:when>
	<c:when test="${WHERE eq 'EDT' }">
		<c:set var="SUC_MSG" value="수정 성공"/>
		<c:set var="FAIL_MSG" value="수정 실패"/>
		<c:set var="SUC_URL" value="/ZAMONG/zamongMagazineView.do?mg_no=${requestScope.mg_no}"/>
	</c:when>
	<c:otherwise>
		<c:set var="SUC_MSG" value="삭제 성공"/>
		<c:set var="FAIL_MSG" value="삭제 실패"/>
		<c:set var="SUC_URL" value="/ZAMONG/zamongMagazine.do"/>
	</c:otherwise>
</c:choose>
